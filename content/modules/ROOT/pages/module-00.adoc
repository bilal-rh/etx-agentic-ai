...
.. https://github.com/bilal-rh/etx-agentic-ai[Fork the etx-agentic-ai repository,window=_blank] to your personal GitHub account
+
.GitHub Repo Fork
...
[source,bash,options="wrap",role="execute"]
----
sed -i "s/bilal-rh/${YOUR_GITHUB_USER}/g" infra/app-of-apps/etx-app-of-apps.yaml
----
...
for x in $(ls infra/app-of-apps/sno); do
    sed -i "s/bilal-rh/${YOUR_GITHUB_USER}/g" infra/app-of-apps/sno/$x
done
----
...
. Now apply the https://github.com/bilal-rh/etx-agentic-ai/tree/main/infra/applications/gpu[GPU Cluster Policy and ConfigMap,window=_blank] objects that setup https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/latest/gpu-sharing.html[Time Slicing,window=_blank] - a method to share nvidia gpus.
+
[source,bash,options="wrap",role="execute"]
----
